...
void animate()
{
    ...
    if (AccumDeltaT > 1.0/30) // fixa a atualizacao da tela em 30
    {
        ...
        angulo+=1.0;
        glutPostRedisplay();
        // Não é o ideal. É importante que voce saiba calcular a velocidade pra quantos pixels ele deve andar.
        Tp.x = (Tp.x + 0.5) * TempoTotal; // s + v * t
        if (Tp.x > WINT_WIDTH) { Tp.x = WINT_WIDTH; }
    }
    if (TempoTotal > 5.0)
    {
...
void display( void )
{
    ...
    glPushMatrix(); // se eu tiro o push and pop qualquer transformação vai valer pra qualquer objeto declarado apos, o push and pop servem pra delimitar o efeito da transformação.
        glTranslatef(Tp.x, Tp.y, 0);
